module github.com/rodenas/temporal/infra/temporal/server-image

go 1.23.2

// The Temporal server version is the pin. Bumping it — together with the
// admin-tools image tag and the server image tag — is the Temporal upgrade PR
// (see infra/temporal/AUTHZ.md §10). go.sum is generated by `go mod tidy` in CI.
require go.temporal.io/server v1.27.0
